...
public:
    // 関数
    void Create(const std::vector<D3D12_INPUT_ELEMENT_DESC>& inputLayoutRef, Blob_t* blobsPtr, size_t  rootParamsCBNum, D3D12_CULL_MODE cullmode, BlendMode mode = BlendMode::NONE);
...
private:
    void CreateAllBlendType(const std::vector<D3D12_INPUT_ELEMENT_DESC>& inputLayoutRef, Blob_t* blobsPtr, size_t  rootParamsCBNum, D3D12_CULL_MODE cullmode, const std::string& mapKey);
